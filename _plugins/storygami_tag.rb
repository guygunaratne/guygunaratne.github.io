...
module Jekyll
  class StorygamiTag < Liquid::Tag
    @videoid = nil
    @width = ''
    @height = ''

    def initialize(tag_name, markup, tokens)
      if markup =~ /(?:(?:http?:\/\/)?(?:www.beta.storygami.co\/(?:video\/|watch\?v=)|youtu.be\/)?(\S+)(?:\?rel=\d)?)(?:\s+(\d+)\s(\d+))?/i
        @videoid = $1
        @width = $2 || "853"
        @height = $3 || "480"
      end
    ...
    def render(context)
      ...
      if @videoid
        ...
        video += %Q{<div class="bt-video-container" style="position:relative;padding-bottom:#{padding_bottom};padding-top:30px;height:0;overflow:hidden"><iframe width="#{@width}" height="#{@height}" src="http://beta.storygami.co/video/a999e69ce/#{@videoid}?rel=0" frameborder="0" allowfullscreen></iframe></div>}
      else
...
Liquid::Template.register_tag('storygami', Jekyll::StorygamiTag)